# -*- coding: utf-8 -*-
"""
Created on Sun Sep 27 19:38:53 2020
...
eyeCascadeClassifier = cv2.CascadeClassifier("D:\\Internship Crime Detection\\livenessDetection\\cascade files\\haarcascade_eye.xml")
...
def draw(photo):
    ...
    if eye_image is not None:
        ...
        print("binary eye image =====================>", binary_eye_image)
        print("==========================================================")
        print(binary_eye_image.shape[0]/5)
        print("==========================================================")
    ###############################################################
    ####### Look for circle (the iris). Save coordinates. ########
    relative_iris_coordinates = None
    if binary_eye_image is not None:
        eye_circles = cv2.HoughCircles(binary_eye_image, cv2.HOUGH_GRADIENT, 3, 500, maxRadius = int(binary_eye_image.shape[0]/5))
...
def main():
    ...
    faceCascade = cv2.CascadeClassifier('D:\\Internship Crime Detection\\livenessDetection\\cascade files\\haarcascade_frontalface_alt.xml')
    ...
    while(1):
        ...
        for (x, y, w, h) in faces:
            crop_face = photo[y:y+h,x:x+w]
            finished, iris_pic = draw(crop_face)
